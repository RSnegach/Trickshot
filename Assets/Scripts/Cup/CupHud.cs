...
namespace Trickshot
{
    ...
    ///
    /// The driver members this file reads - the Callout event, ReplayPlaying / ReplaySkipVotes /
    /// ReplaySkipNeeded / VoteSkipReplay, EmoteWheelOpen, LocalBody.Celeb and the SetPieceTaker -
    /// are called directly on <see cref="CupRoundDriver"/>, as is CupBracketView.DrawMini for the
    /// Tab peek. (They were behind a late-bound seam while the partials were written in parallel;
    /// the tree is stable now, so a rename is a compile error rather than a silent feature-off.)
    /// </summary>
    public sealed class CupHud : MonoBehaviour
    {
        ...
        // ================================================================ binding
        /// <summary>Draw this round: subscribes to its events (PhaseChanged / KickResolved / RoundDecided / Callout).</summary>
        ...
        // every machine - the Local / Host path from OnKickResolved, a client's from ApplyState -
        // and Flash is subscribed to that. A second flash here would double the pill.)
        ...
        void Update()
        {
            ...
            bool replay = _driver.ReplayPlaying;
            ...
            // Escape closes the WHEEL rather than opening the pause menu behind it: the wheel is a
            // cup overlay like the bracket peek and the lobby cards, and CupEscape.Owned reads
            // CupEmoteWheel.AnyOpen so PauseMenu skips the same press. Must come before the toggle
            // below, and before the click handling, so nothing else consumes that frame.
            if (CupEmoteWheel.CloseOnEscape(ref _wheelOpen))
            {
        ...
        void DrawAll()
        {
            ...
            if (_wheelOpen && !PauseMenu.Paused)
            {
                ...
                CupEmoteWheel.Draw(LocalCelebration, _input, Celebration.Pages, ref open);
        ...
        {
            string text = null;
            if (_driver.ReplayPlaying)
            {
                if (CanVoteReplay) text = CupText.ClickToSkipVotes(_driver.ReplaySkipVotes, Mathf.Max(_driver.ReplaySkipNeeded, 1));
            }
        ...
        void DrawBracketPeek(bool hasSide, CupSide view)
        {
            ...
            // DrawMini draws its own "No draw yet" hint for a null bracket, so it owns both cases.
            CupBracketView.DrawMini(inner, bracket, StageOf(), highlight, data);
        }
        ...
        /// Celebration.Play on it would write velocities and control flags to kinematic bones for
        /// nothing - the wheel's pick still rides the input frame (SetEmotePick) to the host, whose
        /// body plays it, and the snapshot poses the puppet with it a round trip later.
        /// </summary>
        Celebration LocalCelebration
        {
            get
            {
                if (_driver == null || _driver.Authority == RoundAuthority.Client) return null;
                var b = _driver.LocalBody;
                return b != null ? b.Celeb : null;
            }
        }
    }
}
